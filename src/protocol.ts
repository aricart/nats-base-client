 ...
 */
//@ts-ignore
import { ConnectionOptions, Msg, Payload } from "./types.ts";
//@ts-ignore
import { Transport, TransportFactory } from "./transport.ts";
//@ts-ignore
  ...
  public static connect(
    ...
    return new Promise((resolve, reject) => {
      if (typeof transportFactory !== "function") {
        reject(new Error("transport is not set"));
      }

      let connectTimeout = options.timeout || 20000;
      const timer = setTimeout(() => {
        reject(NatsError.errorForCode(ErrorCode.CONNECTION_TIMEOUT));
      }, connectTimeout);

      function clearConnectTimeout(err?: Error) {
        clearTimeout(timer);
        if (err) {
          reject(err);
        } else {
      ...
      // FIXME: this needs to handle one or multiple servers
      const u = new URL(options.url);
      const hp = { hostname: u.hostname, port: parseInt(u.port, 10) };
      ph.transport.connect(hp.hostname, hp.port, options)
        .then(() => {
          (async () => {
            try {
